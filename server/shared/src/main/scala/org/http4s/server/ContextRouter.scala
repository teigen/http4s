...
import cats.{Monad, Functor, Contravariant}
...
object ContextRouter {
  ...
  class Segment[F[_], A, B](val run: (A, Uri.Path.Segment) => OptionT[F, B]) {
  ...
  object Routable {
    case class Static[F[_], A](tupled: (String, ContextRoutes[A, F])) extends Routable[F, A]
    case class Dynamic[F[_], A, B](segment: Segment[F, A, B], routes: ContextRoutes[B, F])
